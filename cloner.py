...
times = []
log(f"\nSTARTING {MAX_CLONES} CLONES — ALHAMDULILLAH")
...
for i in range(1, MAX_CLONES + 1):
    ...
    try:
        ...
        # Change package name
        yml_path = f"{temp_dir}/apktool.yml"
        if os.path.exists(yml_path):
            with open(yml_path, "r", encoding="utf-8") as f:
                yml = f.read()
            yml = yml.replace("renameManifestPackage: null", f"renameManifestPackage: {new_pkg}")
        ...
        subprocess.run([
            ...
            "--out", final_apk + ".signed", final_apk
        ], check=True)
        os.replace(final_apk + ".signed", final_apk)
